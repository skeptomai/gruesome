...
### ⚠️ CRITICAL: V3 Property Format Specification Compliance

**DO NOT ADD "TWO-BYTE PROPERTY FORMAT" SUPPORT TO V3** - This breaks all commercial Infocom games!

The Z-Machine specification is **crystal clear** that V3 games support **SINGLE-BYTE FORMAT ONLY**:

#### What the Z-Machine Specification Actually Says

**V1-3 Property Format (Section 12.4.1)**:
...
**V4+ Property Format (Sections 12.4.2.1-2.2)**:
- **Two formats available**:
  - If bit 7 set: Two-byte header format (up to 64 bytes)
  - If bit 7 clear: Single-byte format (1-2 bytes)

#### Historical Regression (October 8, 2025)

**Breaking Commit**: `018212b1` - "Add V3 two-byte property format support to interpreter"

**Incorrect Assumption**: *"the Z-Machine spec defines two-byte format for all versions, but commercial V3 games rarely used properties > 4 bytes"*

**This is completely wrong.** The spec explicitly defines different formats for different versions.

**What Actually Happened**:
1. **Grue compiler issue**: Compiler was generating properties > 8 bytes for V3 games
2. **Wrong solution**: Instead of fixing the compiler to respect V3's 8-byte limit, the developer incorrectly assumed the interpreter was incomplete
3. **Spec misreading**: Saw V4+ supports two-byte format and incorrectly concluded this applied to "all versions"
4. **Implementation error**: Copied V4+ two-byte logic into V3 parsing, breaking compatibility with all real Infocom V3 games

#### Impact of the Regression

**Before Fix**: Zork I worked correctly - "take leaflet" → "Taken."
**After Breaking Change**: "take leaflet" → "You can't see any leaflet here!" (property parsing broken)
**Root Cause**: V3 interpreter incorrectly interpreted bit 7 as "two-byte format flag" instead of normal size encoding

#### The Correct Fix

**V3 Property Parsing** (`src/vm.rs:644-649`):
```rust
...
**DO**:
- ✅ Read Z-Machine specification sections carefully (12.4.1 for V3, 12.4.2 for V4+)
- ✅ Test fixes against **commercial Infocom games** (Zork I, etc.) not just compiled games
- ✅ Fix compiler bugs in the compiler, interpreter bugs in the interpreter

**DON'T**:
- ❌ Assume spec compliance issues are interpreter bugs when they may be compiler bugs
- ❌ Copy V4+ logic into V3 without verifying spec compatibility
- ❌ Change interpreter behavior based on compiled game requirements without checking commercial game compatibility

#### Real Solution

The actual bug is likely in the **Grue compiler** - it shouldn't generate properties > 8 bytes for V3 games, since V3 only supports single-byte format (max 8 bytes). The interpreter was correct to work only with V3-compliant property sizes.

#### Files Modified in Fix

- **`src/vm.rs:644-649`** - Restored correct V3-only single-byte property parsing
- **Root cause**: `get_property_info()` function had incorrect two-byte format logic added for V3
...
**Expected Result**: "Taken." (leaflet successfully taken)
**Broken Behavior**: "You can't see any leaflet here!" (property parsing broken)

This regression demonstrates the critical importance of understanding Z-Machine specification version differences and maintaining compatibility with the commercial game ecosystem.
