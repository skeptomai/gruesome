...
pub fn read_text(g: &GameFile, cso: usize) -> Result<String, io::Error> {
    ...
    let mut current_alphabet = crate::game::Alphabets::A0;
    ...
    loop {
        ...
        let pc = crate::game::read_zchars_from_word(&next_chars).unwrap();
        cp+=2;
        
        for c in pc.chars {

            if is_in_abbrev {
                let asi = crate::game::abbrev_string_index(abbrev_table, c) as usize; // word address
                ...
                match c {
                    ...
                    4 => {
                        current_alphabet = crate::game::Alphabets::A1;
                    },
                    5 => {
                        current_alphabet = crate::game::Alphabets::A2;
                    },                    
                    // current char is normal
                    // BUGBUG: is this guard statement correct? [cb]
                    6 ..= 31 => {
                        ss.push(lookup_char(c, &current_alphabet));
                        current_alphabet = crate::game::Alphabets::A0;
                    },
                    _ => {
...
fn lookup_char(c: u8, alphabet : &crate::game::Alphabets) -> crate::game::Zchar {
    // in the published tables, read char mappings start at index 6
    crate::game::ALPHABETMAP[alphabet].as_bytes()[(c as usize) - 6]
}
...
fn read_zchars_from_word(word: &[u8; 2]) -> Result<crate::game::PackedChars, BitReaderError> {
    // start with a word
    let mut br = BitReader::new(word);

    // lop off top bit as designator of 'last chars here'
    let mut pc = crate::game::PackedChars{last: br.read_u8(1)? == 1, chars: [0,0,0]};
