...
impl ZMachineCodeGen {
    ...
    fn is_true_var_opcode(opcode: u8) -> bool {
        match opcode {
            ...
            0xE2 => true, // STOREB (VAR:226 = opcode 2, so 0xE2)
            ...
            0xE8 => true, // PUSH (VAR:232 = opcode 8, so 0xE8) - THE MISSING ONE!
            0xE9 => true, // PULL (VAR:233 = opcode 9, so 0xE9)
            0xEA => true, // SPLIT_WINDOW (VAR:234 = opcode A, so 0xEA)
            0xEB => true, // SET_WINDOW (VAR:235 = opcode B, so 0xEB)
            0xEC => true, // CALL_VS2 (VAR:236 = opcode C, so 0xEC)
            0xED => true, // ERASE_WINDOW (VAR:237 = opcode D, so 0xED)
            0xEE => true, // ERASE_LINE (VAR:238 = opcode E, so 0xEE)
            0xEF => true, // SET_CURSOR (VAR:239 = opcode F, so 0xEF)
            0xF0 => true, // GET_CURSOR (VAR:240 = opcode 10, so 0xF0)
            0xF1 => true, // SET_TEXT_STYLE (VAR:241 = opcode 11, so 0xF1)
            0xF2 => true, // BUFFER_MODE (VAR:242 = opcode 12, so 0xF2)
            0xF3 => true, // OUTPUT_STREAM (VAR:243 = opcode 13, so 0xF3)
            0xF4 => true, // INPUT_STREAM (VAR:244 = opcode 14, so 0xF4)
            0xF5 => true, // SOUND_EFFECT (VAR:245 = opcode 15, so 0xF5)
            0xF6 => true, // READ_CHAR (VAR:246 = opcode 16, so 0xF6)
            0xF7 => true, // SCAN_TABLE (VAR:247 = opcode 17, so 0xF7)
            0xF8 => true, // NOT (VAR:248 = opcode 18, so 0xF8)
            0xF9 => true, // CALL_VN (VAR:249 = opcode 19, so 0xF9)
            0xFA => true, // CALL_VN2 (VAR:250 = opcode 1A, so 0xFA)
            0xFB => true, // TOKENISE (VAR:251 = opcode 1B, so 0xFB)
            0xFC => true, // ENCODE_TEXT (VAR:252 = opcode 1C, so 0xFC)
            0xFD => true, // COPY_TABLE (VAR:253 = opcode 1D, so 0xFD)
