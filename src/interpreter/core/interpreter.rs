...
use crate::interpreter::core::vm::{CallFrame, VM};
use crate::interpreter::display::display_manager::{create_display, DisplayMode};
use crate::interpreter::display::display_trait::ZMachineDisplay;
use crate::interpreter::input::input_v3::V3Input;
use crate::interpreter::input::input_v4::V4Input;
use crate::interpreter::text;
use crate::interpreter::utils::debug_symbols::RoutineNames;
use log::{debug, info};
...
impl Interpreter {
    ...
    fn dump_memory_state(&self) {
        ...
        for i in [0, 0x48, 0x4c, 0x4e, 0x6f, 0x7f] {
            if let Ok(val) = self.vm.read_global(i) {
                let name = crate::interpreter::utils::debug_symbols::get_global_name(i + 0x10)
                    .unwrap_or("");
                if !name.is_empty() {
            ...
            if should_step {
                ...
                if !instruction.operands.is_empty() {
                        ...
                        .enumerate()
                    {
                        print!(" [{i}] ");
                        match op_type {
                            crate::interpreter::core::instruction::OperandType::Variable => {
                                if *value <= 0xFF {
                                    let var_val =
    ...
    pub fn execute_instruction(&mut self, inst: &Instruction) -> Result<ExecutionResult, String> {
        ...
        for (i, &operand) in inst.operands.iter().enumerate() {
            if inst.operand_types[i] == crate::interpreter::core::instruction::OperandType::Variable
            {
        ...
        match inst.form {
            crate::interpreter::core::instruction::InstructionForm::Short => {
                match inst.operand_count {
                    crate::interpreter::core::instruction::OperandCount::OP0 => {
                    ...
                    crate::interpreter::core::instruction::OperandCount::OP1 => {
            ...
            crate::interpreter::core::instruction::InstructionForm::Long => {
                // Check if this is a math operation and route to math module
                if Interpreter::is_math_opcode(inst.opcode, &inst.operand_count) {
            ...
            crate::interpreter::core::instruction::InstructionForm::Variable => {
                match inst.operand_count {
                    crate::interpreter::core::instruction::OperandCount::OP2 => {
            ...
            crate::interpreter::core::instruction::InstructionForm::Extended => {
        ...
        match inst.opcode {
            ...
            0x05 => {
                ...
                if self.vm.game.header.version <= 3 {
                    ...
                    let save_result = match crate::interpreter::quetzal::save::save_game(&self.vm) {
                        Ok(()) => {
            ...
            0x06 => {
                ...
                if self.vm.game.header.version <= 3 {
                    ...
                    let restore_result =
                        match crate::interpreter::quetzal::restore::restore_game(&mut self.vm) {
                            Ok(()) => {
...
mod tests {
    use super::*;
    use crate::interpreter::core::vm::Game;
