use crate::{
    game::GameFile,
    util::{get_mem_addr, lookup_char, read_zchars_from_word, Alphabets, ZTextReader},
};
use std::{
...
impl Display for Dictionary {
    ...
    fn fmt(&self, f: &mut Formatter) -> Result<(), Error> {
        ...
        writeln!(
            ...
            self.n, self.entry_length, self.number_of_entries
        )?;
        write!(f, "separators: ")?;
        ...
        loop {
            ...
            for c in &pc {
                ...
                if is_in_abbrev {
                    ...
                    log::debug!(
                        "abbrev table {}, index {}, resulting offset: {}",
                        abbrev_table,
                        c,
                        asi
                    );
                    ...
                    unsafe {
                        ss.append(
                            Dictionary::read_text(g, abbrev_string_addr)
                                .unwrap()
                                .as_mut_vec(),
                        )
                    };
