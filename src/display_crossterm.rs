...
impl CrosstermDisplay {
    ...
    pub fn new() -> Result<Self, String> {
        ...
        debug!(
            "CrosstermDisplay: Crossterm reports {}x{}, actual visible {}x{}, offset: {}",
    ...
    fn print_char_upper_window(&mut self, ch: char) -> Result<(), String> {
        if self.upper_window_lines == 0 {
            debug!(
                "Attempted to print '{}' to upper window but no upper window exists",
                ch
            );
        ...
        debug!(
            "Upper window: printing '{}' at ({}, {}) in {}-line upper window",
        ...
        match ch {
            ...
            _ => {
                ...
                while self.upper_window_content.len() <= self.upper_cursor_row as usize {
                    self.upper_window_content
                        .push(vec![' '; self.terminal_width as usize]);
                }
    ...
    fn redraw_upper_window(&mut self) -> Result<(), String> {
        ...
        let lines_to_draw = self
            .upper_window_lines
            .min(self.upper_window_content.len() as u16);
...
impl ZMachineDisplay for CrosstermDisplay {
    ...
    fn split_window(&mut self, lines: u16) -> Result<(), DisplayError> {
        ...
        debug!(
            "Split window: {} lines, cleared upper area, lower window starts at row {}",
            lines, lower_window_top
        );
    ...
    fn set_window(&mut self, window: u8) -> Result<(), DisplayError> {
        ...
        debug!(
            "Set window: {} -> {} (upper_window_lines={})",
            old_window, window, self.upper_window_lines
        );
    ...
    fn set_cursor(&mut self, line: u16, column: u16) -> Result<(), DisplayError> {
        if self.current_window == 1 {
            ...
            debug!(
                "Set upper window cursor: ({}, {})",
                self.upper_cursor_row, self.upper_cursor_col
            );
        } else {
    ...
    fn erase_window(&mut self, window: i16) -> Result<(), DisplayError> {
        match window {
            ...
            0 => {
                ...
                debug!(
                    "Erase window 0: clearing lower window from row {} to {}",
                    lower_start,
    ...
    fn handle_resize(&mut self, width: u16, height: u16) {
        ...
        self.upper_cursor_row = self
            .upper_cursor_row
            .min(self.upper_window_lines.saturating_sub(1));
    ...
    fn set_text_style(&mut self, style: u16) -> Result<(), DisplayError> {
        ...
        debug!(
            "Set text style: 0x{:04x} (reverse: {})",
            style, self.reverse_video
        );
        Ok(())
    }
