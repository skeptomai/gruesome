...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Found {} additional routines referenced in data structures",
        data_routines.len()
    );
...
fn find_data_referenced_routines(game: &Game, known_routines: &HashSet<u32>) -> HashSet<u32> {
    ...
    info!(
        "  Found {} routines in object properties",
        obj_routines.len()
    );
    ...
    info!(
        "  Found {} routines in global variables",
        global_routines.len()
    );
    ...
    info!(
        "  Found {} potential routines in memory scan",
        mem_routines.len()
    );
...
fn scan_property_table(
    ...
    mut addr: usize,
    version: u8,
    found: &mut HashSet<u32>,
    known_routines: &HashSet<u32>,
) {
    // Skip object name
    if addr >= memory.len() {
        return;
        ...
        if version <= 3 {
            ...
            if prop_size >= 2 && addr + 2 <= memory.len() {
                ...
                if let Some(routine_addr) = unpack_routine_address(word, version) {
                    if is_valid_routine_header(memory, routine_addr, version)
                        && !known_routines.contains(&routine_addr)
                    {
...
fn scan_global_variables(game: &Game, known_routines: &HashSet<u32>) -> HashSet<u32> {
    ...
    for i in 0..240 {
        ...
        if let Some(routine_addr) = unpack_routine_address(word, game.header.version) {
            if is_valid_routine_header(&game.memory, routine_addr, game.header.version)
                && !known_routines.contains(&routine_addr)
            {
                found.insert(routine_addr);
                info!(
                    "  Found routine {:05x} in global variable {}",
                    routine_addr,
                    i + 16
                );
            }
