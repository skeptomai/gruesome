...
fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Ok(inst) = Instruction::decode(
            ...
            if inst.opcode == 0x0d {
                ...
                if inst.operands.len() >= 2 {
                    ...
                    if (0x58..=0x5a).contains(&var) {
                        println!(
                            "TIMER INIT at PC 0x{pc:04x}: store G{} = {value}",
                            var - 0x10
                        );
            ...
            if matches!(inst.opcode, 0x19 | 0x00 | 0x0c) && inst.operands.len() >= 3 {
                ...
                if inst.operands[1] > 50 && inst.operands[1] < 500 {
                    ...
                    println!("\nPotential QUEUE call at PC 0x{pc:04x}:");
                    println!("  Routine called: 0x{routine_called:04x}");
            ...
            if inst.opcode == 0x04 && inst.operands.len() >= 4 {
                ...
                if time > 0 && routine > 0 {
                    println!("\nTIMED SREAD at PC 0x{pc:04x}:");
                    println!("  Time: {time} tenths of seconds");
                    println!("  Interrupt routine: 0x{routine:04x}");

                    // Try to trace what the interrupt routine does
                    let int_addr = (routine as u32) * 2;
                    println!("  Interrupt routine unpacked address: 0x{int_addr:04x}");
                }
            }
    ...
    if let Some(queue_addr) = queue_routine_addr {
        println!("\nLikely QUEUE routine at: 0x{queue_addr:04x}");
    }
