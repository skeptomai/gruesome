...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("\n1. Extra routines called by TXD routines ({}):", reachable_by_txd.len());
    for &addr in &reachable_by_txd {
        analyze_routine(&game, addr)?;
    }
    
    info!("\n2. Extra routines only called by other extras ({}):", reachable_by_our_extras.len());
    for &addr in &reachable_by_our_extras {
        analyze_routine(&game, addr)?;
    }
    
    info!("\n3. Extra routines not called by anyone ({}):", unreachable.len());
...
fn analyze_routine(game: &Game, addr: u32) -> Result<(), String> {
    ...
    while pc < game.memory.len() && inst_count < 20 {
        match Instruction::decode(&game.memory, pc, game.header.version) {
            Ok(inst) => {
                if inst_count < 3 {
                    info!("    Inst {}: {:?}", inst_count + 1, inst.format_with_version(game.header.version));
                }
    ...
    info!("    Instructions: {}, Return: {}, Jump: {}, Falls through: {}", 
