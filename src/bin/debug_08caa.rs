...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(inst) = infocom::instruction::Instruction::decode(memory, 0x08caa, 3) {
    ...
    let opcode_byte = memory[0x08caa];
    println!("\nOpcode analysis:");
    ...
    if form == "Long" {
        ...
        // This should be inc_chk
        if opcode == 0x05 {
            println!("This should be INC_CHK instruction (2OP:5)");
        } else {
    ...
    println!("\nContext around 0x08caa:");
    let context = debugger.disassemble_range(0x08ca0, 10);
    for line in context {
        let marker = if line.starts_with("08caa") { " --> " } else { "     " };
        println!("{}{}", marker, line);
    }
