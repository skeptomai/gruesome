...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Find extra routines
    let mut extra_routines: Vec<u32> = our_routines.iter()
    ...
    info!("\n=== SUMMARY ===");
    info!("Valid routines: {} ({:.1}%)", valid_count, (valid_count as f64 / extra_routines.len() as f64) * 100.0);
    info!("Invalid routines: {} ({:.1}%)", invalid_count, (invalid_count as f64 / extra_routines.len() as f64) * 100.0);
    info!("Suspicious routines: {} ({:.1}%)", suspicious_count, (suspicious_count as f64 / extra_routines.len() as f64) * 100.0);
    
    if invalid_count > 0 {
        info!("\n⚠️ WARNING: Found {} false positives (invalid routines)", invalid_count);
    }
...
fn validate_routine(memory: &[u8], addr: u32, version: u8) -> ValidationResult {
    ...
    if instruction_count == 0 {
        ...
        ValidationResult::Suspicious(format!("No terminator after {} instructions", instruction_count))
