...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Checking if {:04x} could be reached by fallthrough...", address);
        ...
        match Instruction::decode(&game.memory, check_addr as usize, game.header.version) {
            Ok(inst) => {
                ...
                if next_addr == address {
                    ...
                    let is_return = matches!(inst.opcode, 0x00 | 0x01 | 0x02 | 0x03 | 0x08 | 0x09 | 0x0a | 0x0b) 
                    ...
                    if !is_return && !is_jump && !is_quit {
                        ...
                        println!("  Instruction at {:04x} would naturally continue to {:04x}", check_addr, address);
                        return Ok(());
                    } else {
                        println!("  Instruction at {:04x} is a {:?} - no fallthrough", 
                            check_addr, if is_return { "return" } else if is_jump { "jump" } else { "quit" });
                    }
                }
