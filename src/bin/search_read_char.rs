...
fn main() {
    ...
    for i in 0..data.len() {
        ...
        if byte == 0xF6 {
            info!(
                "Found potential VAR:0x16 at offset {:04x} (byte: {:02x})",
                i, byte
            );
        ...
        // Also check if it might appear as a 2OP variable form
            ...
            info!(
                "Found potential 2OP:0x16 at offset {:04x} (byte: {:02x})",
                i, byte
            );
    ...
    if locations.is_empty() {
        info!(
            "\nGOOD: No read_char (VAR:0x16) usage found in this v{} game!",
            version
        );
        info!("This is expected - read_char was introduced in Z-Machine v4.");
    } else {
        info!(
            "\nFound {} potential read_char references:",
            locations.len()
        );
        ...
        if version <= 3 {
            info!(
                "\nWARNING: Found potential read_char usage in v{} game!",
                version
            );
    ...
    for (opcode, name) in &v4_opcodes {
        ...
        if count > 0 {
            info!(
                "  Found {} occurrences of byte {:02x} (potential {})",
                count, opcode, name
            );
            v4_found = true;
        }
    ...
    if !v4_found {
        info!(
            "  No v4+ specific opcode bytes found (good for v{} game)",
            version
        );
    }
}
