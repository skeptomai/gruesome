...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &addr in &sorted_routines {
        match get_routine_bounds(&game, addr) {
            ...
            Err(e) => {
                rejection_reasons.entry(addr).or_insert_with(Vec::new)
    ...
    let called_routines: HashSet<u32> = call_graph.values()
    ...
    for &addr in &sorted_routines {
        ...
        if let Ok(quality) = check_routine_quality(&game, addr) {
            ...
            if quality.falls_through && quality.instruction_count < 5 {
                reasons.push(format!("Falls through after {} instructions", quality.instruction_count));
            }
        }
