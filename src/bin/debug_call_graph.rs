...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Uncalled routines: {}", all_routines.len() - called_routines.len());
    ...
    info!("Found {} call targets not in our routine list:", missing_targets.len());
            ...
            Ok(inst) => {
                ...
                match (inst.form, inst.opcode) {
                    ...
                    (InstructionForm::Short, 0x06) => {
                        // call_2n (v5+)
                        if game.header.version >= 5 && !inst.operands.is_empty() && inst.operands[0] != 0 {
                            let unpacked = unpack_address(inst.operands[0], game.header.version);
                            targets.insert(unpacked);
                        }
                    }
                    (InstructionForm::Short, 0x07) => {
                        // call_2s (v4+)
                        if game.header.version >= 4 && !inst.operands.is_empty() && inst.operands[0] != 0 {
                            let unpacked = unpack_address(inst.operands[0], game.header.version);
                            targets.insert(unpacked);
                        }
                    }
