...
fn test_game(
    ...
    let txd_routines = if std::path::Path::new(txd_file).exists() {
        ...
        info!(
            "{}: TXD file not found, using expected count {}",
            game_name, expected_txd_count
        );
        None
    };
    ...
    if let Some(txd_set) = &txd_routines {
        ...
        if !missing.is_empty() {
            ...
            info!(
                "✅ All {} TXD routines found by our disassembler",
                txd_set.len()
            );
        }

        // Check extra routines we find
        let extra: Vec<u32> = our_routines
            .iter()
        ...
        for &addr in &extra[..5.min(extra.len())] {
            if !verify_routine_decodes(&memory, addr, game.header.version) {
                false_positives += 1;
                info!(
                    "  ❌ {:05x} - Does not decode properly (false positive)",
                    addr
                );
            } else {
        ...
        Ok(inst) => {
            ...
            match (inst.form, inst.opcode) {
                ...
                (InstructionForm::Short, 0x00..=0x03)
                    if pc == (addr as usize + 1 + (locals as usize * 2)) =>
                {
