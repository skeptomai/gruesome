...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Without orphan detection: {} routines (expected ~448)", routines_without.len());
    ...
    if routines_with.len() < 440 {
        info!("⚠️  WARNING: V3 regression detected! Should find at least 440 routines (TXD baseline)");
    } else {
