...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let missing_13 = vec![
        0x12a04, 0x12b18, 0x12b38, 0x1b0d8, 0x1b980, 0x1bf3c,
        0x1d854, 0x1da50, 0x1dc1c, 0x1e138, 0x1f250, 0x20ae8,
        0x2b248,
    ];
...
fn analyze_reference_context(game: &Game, loc: usize, packed_addr: u16) {
    ...
    if loc >= obj_table_addr as usize && loc < globals_addr as usize {
        ...
        // Show context
        info!("    {:05x}: Context: {:02x} {:02x} [{:02x} {:02x}] {:02x} {:02x}", 
             loc,
             if loc >= 2 { game.memory[loc - 2] } else { 0 },
             if loc >= 1 { game.memory[loc - 1] } else { 0 },
             game.memory[loc], game.memory[loc + 1],
             if loc + 2 < game.memory.len() { game.memory[loc + 2] } else { 0 },
             if loc + 3 < game.memory.len() { game.memory[loc + 3] } else { 0 });
...
fn find_object_containing_address(game: &Game, addr: u32) {
    ...
    for obj_num in 1..=1000 {  // Be generous with object count
        ...
        if addr >= prop_addr as u32 && addr < prop_addr as u32 + 1000 {  // Generous property size
