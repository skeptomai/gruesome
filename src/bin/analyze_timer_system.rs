        ...
        if let Ok((inst, _)) = disasm.disassemble_instruction(addr as u32) {
            if inst.opcode == 0x04 && inst.operands.len() >= 4 {
                ...
                if time > 0 && routine > 0 {
                    ...
                    println!("  0x{:04x}: sread with time={} routine=0x{:04x}", 
                             addr, time, routine);
                }
            }
        }
            ...
            if let Ok((inst, _)) = disasm.disassemble_instruction(routine_addr + offset) {
                ...
                for (i, op) in inst.operands.iter().enumerate() {
                    if matches!(inst.operand_types[i], OperandType::Variable) {
                        if *op >= 0x58 && *op <= 0x5a {
                            println!("    Found access to G{} at +0x{:02x}: {}", 
                                     op - 0x10, offset, inst.opcode);
                        }
                    }
                }
            }
            ...
            if inst.opcode == 0x0d { // store
                ...
                if let Ok(output) = disasm.disassemble_range(addr as u32, (addr + 50) as u32) {
                    ...
                    for line in &lines {
                        ...
                        if line.contains("get_prop") || line.contains("put_prop") ||
                           line.contains("storew") || line.contains("loadw") {
