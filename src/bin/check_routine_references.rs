...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("=== CHECKING REFERENCES TO ROUTINE {:05x} ===", routine_addr);
...
fn check_sread_references(memory: &[u8], routine_addr: u32, version: u8) {
    ...
    info!("Looking for packed address: {:04x} (unpacked: {:05x})", packed_addr, routine_addr);
...
fn check_packed_references(memory: &[u8], routine_addr: u32, version: u8) {
    ...
    if locations.is_empty() {
        info!("  Packed address {:04x} not found anywhere in memory", packed_addr);
        info!("  This routine is likely unreferenced dead code");
    } else {
        info!("  Packed address {:04x} found at {} locations:", packed_addr, locations.len());
        for (idx, &loc) in locations.iter().enumerate() {
            if idx < 10 {
                info!("    {:05x}: Context: {:02x} {:02x} [{:02x} {:02x}] {:02x} {:02x}", 
                     ...
                     if loc + 2 < memory.len() { memory[loc + 2] } else { 0 },
                     if loc + 3 < memory.len() { memory[loc + 3] } else { 0 });
            }
        }
        if locations.len() > 10 {
