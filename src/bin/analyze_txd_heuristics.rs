...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Checking why TXD might reject our {} extra routines:\n", extras.len());
...
fn analyze_txd_rejection_reasons(game: &Game, addr: u32, all_routines: &HashSet<u32>) -> Result<Vec<String>, String> {
    ...
    if inside_another {
        ...
        if let Some(&container) = all_routines.iter().find(|&&r| r < addr && addr < r + 1000) {
            ...
            reasons.push(format!("Inside routine {:05x} at offset +{}", container, offset));
        }
    }
...
fn is_routine_called(game: &Game, target: u32, all_routines: &HashSet<u32>) -> Result<bool, String> {
...
fn show_routine_context(game: &Game, addr: u32) -> Result<(), String> {
    ...
    for i in 0..3 {
        ...
        match Instruction::decode(&game.memory, pc, game.header.version) {
            Ok(inst) => {
                info!("  Inst {}: {}", i + 1, inst.format_with_version(game.header.version));
                pc += inst.size;
            }
            Err(e) => {
