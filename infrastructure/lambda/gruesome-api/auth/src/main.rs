...
async fn function_handler(
    ...
    let response = match (method, path) {
        ("POST", "/api/auth/signup") => handlers::handle_signup(event, cognito, dynamodb).await,
        ("POST", "/api/auth/login") => handlers::handle_login(event, cognito).await,
        ("POST", "/api/auth/refresh") => handlers::handle_refresh(event, cognito).await,
        ...
        ("GET", "/api/auth/me") => handlers::handle_me(event, jwt_validator, dynamodb).await,
        ("GET", "/health") => {
